...
static int mfc_get_firmware_version(struct mfc_charger_data *charger, int firm_mode)
{
	...
	switch (firm_mode) {
		case MFC_RX_FIRMWARE:
			...
			pr_info("%s rx major firmware version 0x%x(%d)\n", __func__, version, charger->chip_id);
			...
			pr_info("%s rx minor firmware version 0x%x(%d)\n", __func__, version, charger->chip_id);
			break;
		default:
...
static int mfc_get_chip_id(struct mfc_charger_data *charger)
{
	...
	if (ret < 0) {
		pr_info("%s: keep chip id:%d\n", __func__, charger->chip_id);
		return charger->chip_id;
	}

	if (chip_id == 0x04) {
		charger->chip_id = MFC_CHIP_LSI_S2MIW04;
		pr_info("%s: LSI CHIP(S2MIW04 / 0x%02X)\n", __func__, chip_id);
	} else if (chip_id == 0x40) {
		charger->chip_id = MFC_CHIP_LSI;
		pr_info("%s: LSI CHIP(0x%02X)\n", __func__, chip_id);
	} else { /* 0x20 */
		charger->chip_id = MFC_CHIP_IDT;
		pr_info("%s: IDT CHIP(0x%02X)\n", __func__, chip_id);
	}
	return charger->chip_id;
}
...
static void mfc_set_vout(struct mfc_charger_data *charger, int vout)
{
	...
	case MFC_VOUT_9V:
	case MFC_VOUT_10V:
		if (is_IDT_chip(charger->chip_id))
			mfc_reg_write(charger->client, MFC_VOUT_SET_REG, mfc_idt_vout_val[vout]);
		else if (charger->chip_id == MFC_CHIP_LSI_S2MIW04) {
	...
	pr_info("%s vout(%d, 0x%04X) read = %d mV\n", __func__,
		vout, (data[0] | (data[1] << 8)), mfc_get_adc(charger, MFC_ADC_VOUT));
...
static void mfc_led_control(struct mfc_charger_data *charger, bool on)
{
	int i = 0;
...
static bool WriteWordToMtp(struct mfc_charger_data *charger, u16 StartAddr, u32 data)
{
	int j, cnt;
	u8 sBuf[16] = {0,};
...
static int mfc_check_idt_info_page(struct mfc_charger_data *charger)
{
	u8 i = 0;
	u8 rdata = 0;
	u16 BaseAddr = 0x4000;
...
static void mfc_wpc_fw_update_work(struct work_struct *work)
{
	...
	case SEC_WIRELESS_RX_SDCARD_MODE:
		...
		mfc_otp_update = 1;
		if (is_LSI_chip(charger->chip_id)) {
	...
	case SEC_WIRELESS_RX_BUILT_IN_MODE:
		...
		if (is_LSI_chip(charger->chip_id)) {
			...
			do {
				...
				pr_info("%s %s: repiarEn(%d), pgmCnt(%d), ret(%d)\n",
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		...
		if (charger->wc_tx_enable) {
			...
			pr_info("@Tx_Mode %s Firmware Ver = %x(%d), TX_VOUT = %dmV, TX_IOUT = %dmA, OP_FREQ = %dKHz, TX_OP_FREQ = %dKHZ, %s connected\n",
				__func__, charger->pdata->otp_firmware_ver, charger->chip_id, vout, iout, freq, tx_freq, rx_device_type_str[charger->wc_rx_type]);
			...
			freq = mfc_get_adc(charger, MFC_ADC_OP_FRQ);
			pr_info("%s Firmware Ver = %x(%d) RX_VOUT = %dmV, RX_VRECT = %dmV, RX_IOUT = %dmA, OP_FREQ = %dKHz, IC Rev = 0x%x, IC Font = 0x%x, cable=%d\n",
				__func__, charger->pdata->otp_firmware_ver, charger->chip_id, vout, vrect, iout, freq,
...
static void mfc_mst_routine(struct mfc_charger_data *charger, u8 *irq_src)
{
	u8 data = 0;
...
static irqreturn_t mfc_wpc_irq_thread(int irq, void *irq_data)
{
	...
	if (is_LSI_chip(charger->chip_id) && (charger->mst_off_lock == 1)) {
		pr_info("%s MST Off Lock!\n", __func__);
		return IRQ_NONE;
	}
	...
	if (charger->pdata->cable_type != SEC_WIRELESS_PAD_FAKE) {
#if 1 //jaeho21.song
		mfc_reg_write(charger->client, MFC_INT_A_CLEAR_L_REG, 0xFF); // clear int
		mfc_reg_write(charger->client, MFC_INT_A_CLEAR_H_REG, 0xFF); // clear int
#else
		mfc_reg_write(charger->client, MFC_INT_A_CLEAR_L_REG, irq_src[0]); // clear int
		mfc_reg_write(charger->client, MFC_INT_A_CLEAR_H_REG, irq_src[1]); // clear int
#endif
