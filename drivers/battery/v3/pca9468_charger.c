...
static int pca9468_charge_adjust_ccmode(struct pca9468_charger *pca9468)
{
	...
	case CCMODE_CHG_LOOP:
		/* Check TA current */
		if (pca9468->ta_cur > (PCA9468_TA_MIN_CUR * pca9468->ta_mode)) {
...
static int pca9468_preset_dcmode(struct pca9468_charger *pca9468)
{
	...
	int ret = 0;
	int ta_mode;
#if defined(CONFIG_BATTERY_SAMSUNG) //temp
	union power_supply_propval value = {0, };
#endif
...
#if defined(CONFIG_BATTERY_SAMSUNG) //temp
	value.intval = (ta_mode == TA_4TO1_DC_MODE) ? DC_45W : DC_25W;
	psy_do_property("battery", set,
		POWER_SUPPLY_EXT_PROP_DIRECT_POWER_TYPE, value);
#endif
