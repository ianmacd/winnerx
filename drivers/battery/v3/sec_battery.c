...
int sec_bat_set_charging_current(struct sec_battery_info *battery)
{
	...
	if (battery->dc_float_voltage_set) {
		pr_info("%s : step float voltage = %d \n", __func__,
			battery->pdata->dc_step_chg_val_vfloat[battery->step_charging_status+battery->step_45w]);
		value.intval = battery->pdata->dc_step_chg_val_vfloat[battery->step_charging_status+battery->step_45w];
		psy_do_property(battery->pdata->charger_name, set,
...
static void sec_bat_wireless_uno_cntl(struct sec_battery_info *battery, bool en)
{
	...
	if (value.intval) {
		...
		value.intval = en;
		psy_do_property(battery->pdata->vdivider_name, set,
			POWER_SUPPLY_EXT_PROP_WIRELESS_TX_ENABLE, value);
#endif
	} else {
		psy_do_property(battery->pdata->wireless_charger_name, set,
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_CURRENT_EVENT:
			...
			break;
#if defined(CONFIG_DIRECT_CHARGING)
		case POWER_SUPPLY_EXT_PROP_DIRECT_POWER_TYPE:
			if (val->intval == DC_45W)
				battery->pd_list.support_45w = true;
			else
				battery->pd_list.support_45w = false;
			pr_info("%s: set support_45w %d", __func__, val->intval);
#if 0  //temp
			value.intval = (battery->pd_list.support_45w) ? 2 : 1;
			psy_do_property(battery->pdata->vdivider_name, set,
		...
		unsigned long action, void *data)
{
	...
	int fpdo_power = 0;
#if defined(CONFIG_VDIVIDER) //temp
	union power_supply_propval val = {0, };
#endif
#endif
...
#if defined(CONFIG_STEP_CHARGING)
#if defined(CONFIG_DIRECT_CHARGING)
		} else if (is_pd_apdo_wire_type(battery->wire_status) && bPdIndexChanged &&
			battery->pdic_info.sink_status.power_list[current_pdo].apdo) {
#if defined(CONFIG_VDIVIDER)
			val.intval = (battery->pd_list.support_45w) ? 2 : 1;
			psy_do_property(battery->pdata->vdivider_name, set,
				POWER_SUPPLY_PROP_CHARGE_TYPE, val);
#endif
			if (battery->pd_list.support_45w)
				sec_bat_check_dc_step_charging(battery);
			pr_info("%s : support 45w (%d)\n", __func__, battery->pd_list.support_45w? 1 : 0);
#endif
#endif
		}
#endif
