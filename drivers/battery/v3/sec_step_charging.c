...
{
	int i, value, prev_step_45w, dc_step_chg_step;
	int step = -1, step_vol = -1, step_input = -1, step_soc = -1, soc_condition = 0, step_offset = 0;
	...
	pr_info("%s : 45w (%d), (%d), (%d)\n", __func__,
		battery->pdic_info.sink_status.power_list[battery->pdic_info.sink_status.current_pdo_num].max_voltage,
		battery->pdic_info.sink_status.current_pdo_num, battery->pd_list.support_45w);

	prev_step_45w = battery->step_45w;
	if (battery->pd_list.support_45w && (battery->dc_step_chg_step_45w > 0)) {
		battery->step_45w = battery->dc_step_chg_step;
		dc_step_chg_step = battery->dc_step_chg_step_45w;
	} else {
		battery->step_45w = 0;
		dc_step_chg_step = battery->dc_step_chg_step;
	}
	step_offset  = battery->step_45w;

	if (prev_step_45w != battery->step_45w) {
		battery->step_charging_status = -1;
		/* need to change v_float_max 
		dc chg done is possible only when v_float_max value is the same as the last float voltage.
		*/
		val.intval = battery->pdata->dc_step_chg_val_vfloat[dc_step_chg_step-1+step_offset];
		psy_do_property(battery->pdata->charger_name, set,
	...
	if (battery->dc_step_chg_type & STEP_CHARGING_CONDITION_SOC) {
		...
		if (battery->step_charging_status >= 0 &&
			(battery->siop_level < 100 || battery->lcd_status)) {
			soc_condition = battery->pdata->dc_step_chg_cond_soc[step_soc+step_offset] + DIRECT_CHARGING_FORCE_SOC_MARGIN;
	...
	if (battery->dc_step_chg_type & STEP_CHARGING_CONDITION_VOLTAGE) {
		...
		while(step_vol < dc_step_chg_step - 1) {
			if (value < battery->pdata->dc_step_chg_cond_vol[step_vol+step_offset])
				break;
			step_vol++;
			if (battery->step_charging_status >= 0)
				break;
	...
	if (battery->dc_step_chg_type & STEP_CHARGING_CONDITION_INPUT_CURRENT) {
		...
		if (val.intval != SEC_DIRECT_CHG_MODE_DIRECT_ON) {	
			...
			while(step_input < dc_step_chg_step - 1) {
				if (value > battery->pdata->dc_step_chg_cond_iin[step_input+step_offset])
					break;
				step_input++;
...
check_dc_step_change:
	...
	if (battery->step_charging_status < 0 ||
		...
		pr_info("%s : cable(%d), step changed(%d->%d), current(%dmA)\n",
			...
			battery->step_charging_status, step, battery->pdata->dc_step_chg_val_iout[step+step_offset]);
			battery->pdata->charging_current[battery->cable_type].fast_charging_current = battery->pdata->dc_step_chg_val_iout[step+step_offset];

		if ((battery->dc_step_chg_type & STEP_CHARGING_CONDITION_FLOAT_VOLTAGE) &&
			(battery->swelling_mode == SWELLING_MODE_NONE)) {
			if (battery->step_charging_status < 0) {
				pr_info("%s : step float voltage = %d \n", __func__, battery->pdata->dc_step_chg_val_vfloat[step+step_offset]);
				val.intval = battery->pdata->dc_step_chg_val_vfloat[step+step_offset];
				psy_do_property(battery->pdata->charger_name, set,
		...
		if (battery->dc_step_chg_type & STEP_CHARGING_CONDITION_INPUT_CURRENT) {
			if (battery->step_charging_status < 0) {
				pr_info("%s : step input current = %d \n", __func__,
					battery->pdata->dc_step_chg_val_iout[step+step_offset] / 2);
				val.intval = battery->pdata->dc_step_chg_val_iout[step+step_offset] / 2;
				psy_do_property(battery->pdata->charger_name, set,
...
int sec_dc_step_charging_dt(struct sec_battery_info *battery, struct device *dev)
{
	...
	ret = of_property_read_u32(np, "battery,dc_step_chg_step_45w",
			&battery->dc_step_chg_step_45w);
	if (ret) {
		pr_err("%s: dc_step_chg_step is Empty\n", __func__);
		battery->dc_step_chg_step_45w = 0;
	} else {
		pr_err("%s: dc_step_chg_step_45w is %d\n",
			__func__, battery->dc_step_chg_step_45w);
	}

	battery->dc_step_chg_len = battery->dc_step_chg_step + battery->dc_step_chg_step_45w;
...
/* [dchg] TODO: do some error handling */
	...
	for (i = 0; i < battery->dc_step_chg_len; i++) {
...
#if defined(CONFIG_DIRECT_CHARGING)
	union power_supply_propval val;
	int dc_step_chg_step, i = 0, step_offset = 0;
...
#if defined(CONFIG_DIRECT_CHARGING)
		for (i = 0; i < dc_step_chg_step; i++) {
			if (battery->pdata->dc_step_chg_val_vfloat[i+step_offset] > battery->pdata->chg_float_voltage)
				battery->pdata->dc_step_chg_val_vfloat[i+step_offset] = battery->pdata->chg_float_voltage;
			if (battery->pdata->dc_step_chg_cond_vol[i+step_offset] > battery->pdata->chg_float_voltage)
				battery->pdata->dc_step_chg_cond_vol[i+step_offset] = battery->pdata->chg_float_voltage;
		}
		for (i = 0; i < dc_step_chg_step; i++)
			dev_info(battery->dev, "%s: cond_vol: %dmV, vfloat: %dmV\n", __func__,
		 		battery->pdata->dc_step_chg_cond_vol[i+step_offset], battery->pdata->dc_step_chg_val_vfloat[i+step_offset]);
		val.intval = battery->pdata->dc_step_chg_val_vfloat[dc_step_chg_step-1+step_offset];
		psy_do_property(battery->pdata->charger_name, set,
			POWER_SUPPLY_EXT_PROP_DIRECT_FLOAT_MAX, val);
#endif
